...
### Mocks
The mocking ended up being reasonable complex.  Because other areas of the testing (in the unit tests) have mocks that mock an entire module in global scope & because we need some functions to only be mocked some of the time it was necessary to resort to [`jest.isolateModules`](https://jestjs.io/docs/jest-object#jestisolatemodulesfn) in `ITestPageRequester` to build a parallel module registry.  Whilst this does incur a minor overhead (measured to be on the order of single digit ms) it is one of the only ways to sidesteps the mocking overreach that is present in global scope.  The only other reasonable alternative is to make the integration tests ran in a separate process loaded with a different tsconfig however that would be even more heavy handed and so this solution was selected.
